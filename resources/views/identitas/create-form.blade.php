        ...
        <div class="col-md-10">
            <form action="/identitas" method="post" enctype="multipart/form-data">
                ...
                <div id="data-pns" style="display: none">
                    ...
                    <div class="mb-3">
                        <label for="role_id" class="form-label">Roles <span
                                class="text-danger">*</span></label>
                        <select class="form-select @error('role_id') is-invalid @enderror"
                            value="{{old('role_id')}}" aria-label="roles" id="role_id"
                            name="role_id">
                            <option value="">Pilih Roles</option>
                            @foreach ($rowsRole as $rowRole)
                            <option {{old('role_id')==$rowRole['role_id'] ? 'selected' : '' }}
                                value="{{ $rowRole['id'] }}">{{ $rowRole['role_name'] }}
                            </option>
                            @endforeach
                        </select>
                        @error('role_id')
